...
完成形のコードは [GitHub リポジトリ](https://github.com/) で公開しています（※準備中）。本章では実装の核となる手順を取り上げます。
...
設計の鍵は、**最初から大規模を想定して過剰に複雑化させない**ことです。SSGで始めて、必要に応じてSSRに移行するのがAstroの合理的なアプローチです。
...
次章では、サイドバーや目次生成を備えたドキュメントサイトの構築手順に進みます。

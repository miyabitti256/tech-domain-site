...
## 動的UIの実装とクライアントディレクティブ

第2章でIslands Architectureの概念を整理しました。本章では、ReactやVueなどのUIコンポーネントをAstroに組み込み、必要な箇所だけをインタラクティブ化する実践手順を解説します。
...
Astroでは、**1つのページ内に複数のUIフレームワークのコンポーネントを混在して共存**させられます。

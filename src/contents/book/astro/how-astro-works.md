...
Astroが高速なWebサイトを実現できる理由は、HTML生成とJavaScript配信の分離にあります。この仕組みを把握しておくと、コンポーネント設計やディレクティブの使い分けに迷わなくなります。
...
最大の特徴は、**ページの描画処理がすべてビルド時に完結している**点にあります。ブラウザは完成したHTMLを受け取るだけで即座にページを表示できるため、初回表示が極めて高速です。

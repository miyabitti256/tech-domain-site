...
#### View Transitionsとの共存問題

はじめは、Astroの[View Transitions](https://docs.astro.build/en/guides/view-transitions/)の疑似要素で再現しようと考えていましたが、View Transitionsが有効だと、ブラウザがページ遷移時にDOMのスナップショットを撮ってクロスフェードを行う動作が、テーマ切り替え時にこのスナップショットと実際のDOMの間で**ちらつき**が発生してしまうため、実DOMで再現する方法に切り替えました。また、テーマ切り替え処理中はカーテン要素以外のアニメーションを無効化しています。

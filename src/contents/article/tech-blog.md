...
Astroを選んだ最大の理由は、**デフォルトでクライアントサイドJSがゼロ**という設計思想です。
以前はSvelteKitでサイトを運用した経験もありました。しかし、大半が静的コンテンツであるブログであれば、クライアント側に重いランタイムを抱え込む必要はありません。
Astroのアイランドアーキテクチャなら、動的なUIが必要なパーツだけに絞ってJSを配信できます。不要なスクリプトはビルド時に削ぎ落とされるため、極めて軽量な配信が可能です。

加えて、Content Collectionsによる型安全な記事管理や、View Transitionsによる滑らかなページ遷移など、ブログ開発に必要な機能が標準で充実していた点も決め手になりました。
